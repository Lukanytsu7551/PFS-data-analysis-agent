...
async function openMcpSettings() {
  ...
  try {
    ...
    } else if (pfs()?.sidebar?.openPanel) {
      pfs().sidebar.openPanel("mcp");
    }
  } finally {
...
function toggleMcpAddForm() {
  if (pfs() && getUiIsland("mcp")) getUiIsland("mcp").toggleForm();
}

function openMcpEditForm(server) {
  if (pfs() && getUiIsland("mcp")) {
...
function renderMcpServerList(servers) {
  if (pfs() && getUiIsland("mcp")) getUiIsland("mcp").setServers(servers);
}
...
async function connectMcpServer(serverId) {
  if (pfs() && getUiIsland("mcp")) getUiIsland("mcp").updateServer(serverId, { busy: true });
  try {
    ...
    if (pfs() && getUiIsland("mcp")) getUiIsland("mcp").updateServer(serverId, { busy: false });
  }
}
