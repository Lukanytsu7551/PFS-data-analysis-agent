...
export function mountWorkspaceUi() {
    ...
    return h("div", { class: "ws-state-card" }, [
      ...
      h("div", { class: "ws-state-actions" }, [
        ...
        h("button", {
          ...
          disabled: state.busy,
          onClick: () => {
            if (pfs()?.workspace && typeof pfs().workspace.doUnmount === "function") {
              pfs().workspace.doUnmount();
            }
          },
            ...
            return h("div", {
              ...
              editing ? null : h("div", { class: "ws-known-actions" }, [
                h("button", {
                  ...
                  onClick: () => pfs()?.workspace?.activateKnownWorkspace?.(workspace),
                }, workspace.current
  ...
  async function _saveWorkspaceRename(workspace) {
    ...
    _render();
    try {
      await pfs()?.workspace?.renameKnownWorkspace?.(workspace.workspace_id, name);
  ...
  async function _removeKnownWorkspace(workspace) {
    ...
    _render();
    try {
      await pfs()?.workspace?.removeKnownWorkspace?.(workspace);
    } catch (error) {
      state.knownError = String(error.message || error);
      pfs()?.overlay?.toast?.(state.knownError, "err");
    } finally {
  ...
  async function _cleanupWorkspaceStorage(workspace) {
    ...
    _render();
    try {
      await pfs()?.workspace?.cleanupWorkspaceStorage?.(workspace);
    } catch (error) {
      state.knownError = String(error.message || error);
      pfs()?.overlay?.toast?.(state.knownError, "err");
    } finally {
