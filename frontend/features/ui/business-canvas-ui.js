      ...
      _protocol = initDrawioProtocol(iframe, {
        ...
        onAutosave(xml) {
          if (state.currentProject?.id) {
            pfs().businessCanvas?.saveDiagramXml?.(state.currentProject.id, xml);
          }
        },
        onExport(result) {
  ...
  function _renderSidebar() {
    ...
    const templateButtons = templates.map(tpl =>
      h("button", {
        ...
        type: "button",
        key: `tpl-${tpl.id}`,
        onClick: () => pfs().businessCanvas?.createProject?.(tpl.id),
      }, [
    ...
    const projectItems = projects.length
      ? projects.map(p => {
          ...
          return h("button", {
            ...
            type: "button",
            key: `proj-${p.id}`,
            onClick: () => pfs().businessCanvas?.selectProject?.(p.id),
          }, [
                ...
                ? h("input", {
                    ...
                    onKeydown: (e) => {
                      if (e.key === "Enter") {
                        ...
                        if (t) pfs().businessCanvas?.renameProject?.(p.id, t);
            ...
            h("span", {
              ...
              onClick: (e) => {
                e.stopPropagation();
                pfs().businessCanvas?.deleteProject?.(p.id);
              },
            }, "×"),
          ]);
        })
              ...
              onClick: async () => {
                if (window.confirm(text("canvas.clear_confirm", "确定要清空全部最近项目？此操作不可撤销。"))) {
                  for (const p of [...projects]) {
                    await pfs().businessCanvas?.deleteProject?.(p.id);
                  }
                }
              },
          ...
          onKeydown: (e) => {
            if (e.key === "Enter") {
              ...
              if (newTitle && state.currentProject?.id) {
                pfs().businessCanvas?.renameProject?.(state.currentProject.id, newTitle);
              }
    ...
    return h("div", { class: "dw-toolbar", key: "toolbar" }, [
      ...
      h("div", { class: "dw-toolbar-actions", key: "actions" }, [
        ...
        h("button", {
          ...
          type: "button",
          key: "btn-fullscreen",
          onClick: () => pfs().businessCanvas?.setFullscreen?.(!state.fullscreen),
        }, state.fullscreen ? text("canvas.exit_fullscreen", "退出全屏") : text("canvas.fullscreen", "全屏")),
        h("button", {
          class: "btn-sm btn-sm-ghost",
          type: "button",
          key: "btn-close",
          onClick: () => pfs().businessCanvas?.close?.(),
