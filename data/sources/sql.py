...
# Override via env var: PFS_LAZY_LOAD_LIMIT=200000
_LAZY_LOAD_ROW_LIMIT = int(env("PFS_LAZY_LOAD_LIMIT", "500000"))
...
class SQLDataSource(DataSource):
    ...
    def _tables_in_sql(self, sql: str) -> List[str]:
        ...
        The returned list contains both selected source-table names and
        unselected/unknown table references.  Keeping unknown references in
        the result is important: ``_assert_analysis_scope`` must reject a
        query against a system table instead of treating it as a query with
        no table references and falling back to the remote database.

        Environments without a working sqlglot installation fail closed for
        statements with a FROM/JOIN clause; a substring match is not safe
        because a known table name may occur inside a literal or comment.
        """
        ...
        try:
            ...
            parsed = sqlglot.parse(sql, dialect="duckdb", error_level=sqlglot.ErrorLevel.RAISE)
            found: Set[str] = set()
            for stmt in parsed:
                if stmt is None:
                    continue
                # A CTE is represented as an exp.Table too when it is
                # referenced by the outer query.  It is an internal alias,
                # not a source table that needs scope authorization.
                cte_names = {
                ...
                for tbl_node in stmt.find_all(exp.Table):
                    ...
                    if not tbl_name or tbl_name.lower() in cte_names:
                        continue
                    # Preserve a schema/catalog qualifier for an unknown
                    # reference so the scope error remains diagnosable.  A
                    # known table is normalized to the catalog's spelling so
                    # downstream cache loading keeps working.
                    qualified = tbl_name
                    db_name = str(getattr(tbl_node, "db", "") or "").strip()
                    catalog_name = str(getattr(tbl_node, "catalog", "") or "").strip()
                    if db_name:
                        qualified = f"{db_name}.{qualified}"
                    if catalog_name:
                        qualified = f"{catalog_name}.{qualified}"
                    match = next(
                        (name for name in known if name.lower() == tbl_name.lower()),
                        None,
                    )
                    found.add(match or qualified)

            return list(found)
        except Exception as exc:
            log.debug("[SQLDataSource] sqlglot table extraction failed (%s), using heuristic", exc)

        # ── Fallback: fail closed when table references are not parseable ──────
        sql_upper = sql.upper()
        # No parser + a FROM/JOIN clause is not safe to pass through because
        # we cannot establish that every referenced table is in scope.
        if re.search(r"\b(?:FROM|JOIN|UPDATE|INTO|DELETE\s+FROM)\b", sql_upper):
            return ["<无法识别的 SQL 表引用>"]
        return []
