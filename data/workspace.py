...
from infrastructure.compat import workspace_hidden_dir, workspace_metadata_dir
...
class WorkspaceRuntime:
    ...
    """
    workspace_id: str                    # 来自 .pfs/workspace.json 的稳定 UUID
    ...
    cache_dir: Path = field(init=False)       # Parquet 缓存目录（workdir/.pfs_cache）
    meta_dir: Path = field(init=False)        # 元数据目录（workdir/.pfs）—— 持久化 DB + registry
    ...
    def __post_init__(self):
        ...
        self.cache_dir = workspace_hidden_dir(self.workdir, ".pfs_cache")
        self.meta_dir = workspace_metadata_dir(self.workdir)
    ...
    def list_data_files(self, max_files: int = 50) -> List[dict]:
        ...
        跳过 artifacts/、.pfs/、.pfs_cache/、隐藏目录/文件、黑名单路径。
        """
        results: List[dict] = []
        max_files = max(1, int(max_files))
        skip_dirs = {
            "artifacts", ".pfs", ".pfs_cache", "__pycache__",
            "node_modules", ".venv",
        }
        try:
