...
def is_provider_switchable(exc: Exception) -> bool:
    """Return whether a failed provider call may be retried on another provider.

    A context-window or malformed-request error belongs to the current request
    and should not be sent to another provider.  Transient transport/service
    failures and authentication failures, however, can be isolated to the
    selected provider and are eligible for a configured fallback.
    """
    if is_context_length_error(exc):
        return False
