...
`/teams` 和 `/robot` 是已启用的扩展入口。本地团队和配置界面可使用；真实外部服务是否可连接，取决于当前配置和目标环境，本轮不做真实验收。
...
当前交付重点是可下载源码、桌面本地启动和核心数据分析闭环。GitHub Actions run `34191614452` 已验证远端发布候选树 `ae8142c0a6754ed7635eea9ffece550979833d01` 的 Windows x64、macOS Apple Silicon、Windows 干净源码安装、前端质量和两端打包。最新的 SQL 兼容修复已纳入当前发布切片，仍需新的同 SHA CI 后再作为正式候选发布；正式 GitHub Release 仍等待实机、真实业务与 Office 验收，不会仅因许可证文件存在而提前创建。

真实 Windows 电脑和 macOS Apple Silicon 的安装升级卸载、最新 SQL 兼容修复包复测、复杂 Office 原生视觉、真实业务数据验收、真实模型和外部服务、部署及线上验收仍需在对应环境单独完成。本轮不进行真实外部服务验收，不阻塞本地首版。

本地测试、固定样例或配置文件只能证明对应层级，不等同于生产数据、外部服务或线上可用性。已启用入口在没有目标和凭证时不会自动发起外部调用；GPU 本地/远程执行开关保持默认关闭。
