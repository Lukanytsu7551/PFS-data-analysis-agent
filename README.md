...
> 当前版本：`0.1.0-dev`。当前交付以本地桌面运行、结构化数据分析和跨平台启动为重点。
...
| 协作扩展 | 保留 MCP、Teams、Hooks、飞书和云端登录扩展入口，配置后按需使用；GPU/远程执行默认关闭 |
...
商业画布与 Google Sheets 不属于当前产品范围。MCP、Teams、Hooks、飞书和云端登录保留为可选扩展；GPU/远程执行入口保留，但默认关闭，需要用户在本地明确配置后才会尝试运行。
