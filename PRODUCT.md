...
- MCP、Teams、Hooks、飞书和云端登录入口默认可用；GPU/远程执行默认关闭。
...
本文件描述产品结构，不替代运行验收。当前版本重点验证桌面本地启动、核心分析、图表、交付物、任务和工作区基础链路。最近一次成功的 GitHub Actions run `34029259331` 验证了提交 `b2a6393` 的 Windows x64/macOS Apple Silicon 构建及 Windows 干净源码安装；当前 `main` 为 `1cc49c9`，尚未运行同 SHA 的 CI。根目录 [`LICENSE`](LICENSE) 已提供自定义非商业许可，但正式 GitHub Release 仍等待最终功能和业务验收。
