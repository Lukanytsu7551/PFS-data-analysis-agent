(function () {
  ...
  const _LANG_KEY = "pfs_lang";
  const _SESSION_KEY = "pfs_session_id";
  function _readPfsStorage(storage, key) {
    return storage.getItem(key);
  }
  function _detectLang() {
    try { return (_readPfsStorage(localStorage, _LANG_KEY) || navigator.language || "zh").startsWith("en") ? "en" : "zh"; }
    catch { return "zh"; }
  }
  ...
  let sessionId = _urlSid || _readPfsStorage(sessionStorage, _SESSION_KEY) || "";
  if (_urlSid) sessionStorage.setItem(_SESSION_KEY, _urlSid);
  ...
  async function loadDashboard() {
    ...
    const brandLabel = _lang === "en" ? "PFS Data Analysis Agent" : "PFS 数据分析 Agent";
  ...
  async function refreshSingleWidget(widgetId) {
    let sid = sessionId;
    if (!sid) { sid = await promptSessionId(); if (!sid) return; sessionId = sid; sessionStorage.setItem(_SESSION_KEY, sid); }
    const w = dashboard?.widgets?.find(x => x.id === widgetId);
    if (!w) return;
    const body = $(`wb-${w.id}`);
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 404 || resp.status === 400) {
          ...
          sessionId = ""; sessionStorage.removeItem(_SESSION_KEY);
